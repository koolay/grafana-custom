...
RUN PLUGINS_DIR="/var/lib/grafana/plugins" && \
    # 获取 Metrics 插件最新版本
    METRICS_VER=$(curl -sS https://api.github.com/repos/VictoriaMetrics/victoriametrics-metrics-datasource/releases/latest | grep -oE '"tag_name": "v[0-9]+\.[0-9]+\.[0-9]+"' | cut -d'"' -f4) && \
    # 获取 Logs 插件最新版本
    ...
    echo "Installing Metrics: $METRICS_VER, Logs: $LOGS_VER" && \
    # 下载 Metrics 插件
    curl -fsSL -o /tmp/metrics.tar.gz "https://github.com/VictoriaMetrics/victoriametrics-metrics-datasource/releases/download/${METRICS_VER}/victoriametrics-metrics-datasource-${METRICS_VER}.tar.gz" && \
    # 下载 Logs 插件
    ...
    # 解压插件
    tar -xzf /tmp/metrics.tar.gz -C "${PLUGINS_DIR}" && \
